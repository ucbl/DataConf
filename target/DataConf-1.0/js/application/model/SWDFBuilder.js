...
function SWDFBuilder(){
   ...
   // this.sparqlEndPointURL = 'http://posters.www2012.org:8080/sparql/';
    ...
    this.sparqlEndPointURL = ConfigurationManager.getInstance().getDatasourceServiceUri("conferenceDatasource");
    this.dataType          = ConfigurationManager.getInstance().getDatasourceDataType("conferenceDatasource");
    this.output            = '';
    this.method			   = ConfigurationManager.getInstance().getDatasourceMethod("conferenceDatasource");
    this.CorsEnable    = ConfigurationManager.getInstance().getDatasourceCorsEnable("conferenceDatasource");
...
//        this.sparqlEndpoint.setOutput('xml');  // set output if SWDF Sparql Enpoint
  //       this.sparqlEndpoint.setOutput('xml');     // do  not set output if 4store Sparql Endpoint : output did not implement
